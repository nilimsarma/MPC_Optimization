...
// ++ Create Sq_Matrix_A ++
	...
	for(i = 0, k = ( HESSIAN_LAGRANGIAN_SIZE + DIAG_MATRIX_SIGMA_SIZE); i < JACOBIAN_INEQUALITIES_NUM_ROWS ; i++, k++)
	...
	for(i = ( HESSIAN_LAGRANGIAN_SIZE + DIAG_MATRIX_SIGMA_SIZE ), j = JACOBIAN_INEQUALITIES_NUM_COLS; i < SQ_MATRIX_A_SIZE; i++, j++)
	{
		Sq_Matrix_A[i][j] = -1.0;
	}
...
//	print_vector(Vector_x,VECTOR_b_SIZE);
	...
	for(i = 0; i < VECTOR_SIZE_Pw; i++, j++)	s_primal_dual_dir.Vector_Pw[i] = Vector_x[j];
...
void Compute_Diag_Matrix_Sigma(const struct_ip_vars &s_ip_vars, double Diag_Matrix_Sigma[DIAG_MATRIX_SIGMA_SIZE])
{
	...
	{
		Diag_Matrix_Sigma[i] = -s_ip_vars.Lagrange_multiplier_inequality[i] / s_ip_vars.W[i];
	}
}
...
void Compute_vector_b1(const struct_ip_vars &s_ip_vars, double vector_b1[VECTOR_SIZE_b1])
{
	...
	{
		vector_b1[i] = s_ip_vars.Lagrange_multiplier_inequality[i] - s_ip_vars.mu / s_ip_vars.W[i];
	}
}
...
{
	int i;	
	Compute_inequality_constraints(s_ip_vars, vector_b2);

	for(i = 0; i < NUM_INEQUALITY_CONSTRAINTS; i++)
	{
		vector_b2[i] = s_ip_vars.W[i] - vector_b2[i];
	}
}
...
*/
	...
	for(i = 0; i < NUM_INEQUALITY_CONSTRAINTS; i++)
	{
		Lagrangian -= s_ip_vars.Lagrange_multiplier_inequality[i]*(inequality_constraints[i] - s_ip_vars.W[i]);
	}
